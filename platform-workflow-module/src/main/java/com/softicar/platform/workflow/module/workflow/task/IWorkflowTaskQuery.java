...
public interface IWorkflowTaskQuery extends IDbQuery<IWorkflowTaskQuery.IRow> {
	...
	IDbQueryTableColumn<IRow, AGWorkflow> WORKFLOW_COLUMN = new DbQueryTableStubColumn<>(IRow::getWorkflow, "workflow", AGWorkflow.TABLE);
	...
	class Implementation {
		...
		private static class Factory implements IFactory {
			...
			public Factory() {
				...
				this.columns.add(WORKFLOW_COLUMN);
		...
		private static class Query extends AbstractDbQuery<IRow> implements IWorkflowTaskQuery {
			...
			private class QuerySqlBuilder extends AbstractDbQuerySqlBuilder {
				...
				public void buildOriginalSelect() {
					...
					JOIN(SqlKeyword.LEFT);
					addIdentifier(AGWorkflowTaskExecution.TABLE);
					addToken(SqlKeyword.AS);
					addIdentifier("taskExecution");
					ON();
					addIdentifier("taskExecution", AGWorkflowTaskExecution.WORKFLOW_TASK);
					addToken(SqlSymbol.EQUAL);
					addIdentifier("task", AGWorkflowTask.ID);
					JOIN(SqlKeyword.LEFT);
					addIdentifier(AGWorkflowSpecificUserConfiguration.TABLE);
					addToken(SqlKeyword.AS);
					addIdentifier("configuration");
					ON();
					addIdentifier("configuration", AGWorkflowSpecificUserConfiguration.USER);
					addToken(SqlSymbol.EQUAL);
					addParameter(parameters.user);
					addToken(SqlKeyword.AND);
					addIdentifier("configuration", AGWorkflowSpecificUserConfiguration.WORKFLOW);
					addToken(SqlSymbol.EQUAL);
					addIdentifier("item", AGWorkflowItem.WORKFLOW);
					WHERE();
					...
					WHERE();
					addIdentifier("taskExecution", AGWorkflowTaskExecution.ID);
					addToken(SqlKeyword.IS);
					addToken(SqlKeyword.NULL);
					WHERE();
					addToken(SqlKeyword.IFNULL);
					addToken(SqlSymbol.LEFT_PARENTHESIS);
					addIdentifier("configuration", AGWorkflowSpecificUserConfiguration.SUBSCRIBED);
					addToken(SqlSymbol.COMMA);
					addLiteral(true);
					addToken(SqlSymbol.RIGHT_PARENTHESIS);
					if(parameters.showMyDelegations) {
						WHERE();
						addToken(SqlSymbol.LEFT_PARENTHESIS);
		...
		private static class Row extends AbstractDbQueryRow<IRow> implements IRow {
			...
			private Row(IWorkflowTaskQuery query, IDbSqlSelect select, DbResultSet resultSet) {
				...
				this.workflow = WORKFLOW_COLUMN.loadValue(select, resultSet);
