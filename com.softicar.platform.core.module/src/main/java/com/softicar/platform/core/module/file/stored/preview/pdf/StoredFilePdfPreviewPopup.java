...
import com.softicar.platform.core.module.file.stored.preview.LimitedWidthZoomableImageDiv;
...
public class StoredFilePdfPreviewPopup extends AbstractStoredFilePreviewPopup {
	...
	public StoredFilePdfPreviewPopup(AGStoredFile file, ICssLength imageInitialMaxWidth) {
		...
		super(file);
		List<LimitedWidthZoomableImage> previewImages = getPreviewImages(file, imageInitialMaxWidth);
		if (!previewImages.isEmpty()) {
			appendChild(new LimitedWidthZoomableImageDiv(previewImages));
		} else {
			appendChild(new DomMessageDiv(DomMessageType.WARNING, EmfI18n.NO_IMAGE_FOUND_FOR_ARG1.toDisplay(file.getFileName())));
		}
	}

	private List<LimitedWidthZoomableImage> getPreviewImages(AGStoredFile file, ICssLength maxWidth) {
